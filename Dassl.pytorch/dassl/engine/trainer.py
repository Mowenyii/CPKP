...
class SimpleTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        ...
        self.entities_0 = None
        self.entities_1 = None
        self.relations_0 = None
        self.cfg = cfg
        ...
        # self.loss_diff_t_sum_list = []#记录因果贡献
        self.t=5
        self.loss_list = torch.zeros([self.t,8])
        self.fin_loss_list = torch.zeros(8)
    ...
    def after_train(self):
        ...
        if do_test:
            ...
            if self.cfg.TRAINER.COOP.PRUNE:
                # CapKP
                self.test(entities_0=self.entities_0, entity_1=self.entities_1, relation_0=self.relations_0, split=None,
                              eval_no_Prune=None, loss_diff_t=self.fin_loss_list)
            else:
    ...
    def exp_moving_ave(self,theta, beta):
    ...
    def after_epoch(self):
        ...
        with torch.no_grad():
            if self.max_epoch - self.epoch <= self.t :
                # CapKP
                best_loss,image, label=None,None,None
                loss_diff_t_sum = 0
                ...
                for batch_idx, batch in enumerate(self.train_loader_x):
                    ...
                    best_loss = F.cross_entropy(output, label).item()
                    masked_loss = [torch.tensor(best_loss)]
                    for idx in range(1, 8):

                        outputs,fea,fea_agg = self.model.module.forward(image, idx)
                        t_loss = F.cross_entropy(outputs, label).item()
                        masked_loss += [torch.tensor(float(t_loss))]

                    masked_loss = torch.stack(masked_loss)
                    loss_diff = masked_loss - best_loss
                    loss_diff_t = loss_diff.t()
                    loss_diff_t_sum += loss_diff_t


                self.loss_list[int(self.max_epoch - self.epoch-1)]=loss_diff_t_sum


                if self.max_epoch - self.epoch == 1:

                    self.fin_loss_list = torch.cat(
                        [self.exp_moving_ave(self.loss_list[:, i], 0.8)[-1] for i in range(8)], dim=0)
                    weights=self.fin_loss_list

                    save_path = str(
                        self.cfg.OUTPUT_DIR) + "/loss_diff_t_sum.pt"
                    torch.save(weights, save_path)
                    print("loss_diff_t_sum, save path", weights, save_path)
                    ...
                    for j in range(len(entities_0)):
                        ...
                        sub_edge_idxs = np.argsort(sub_weights)
                        ...
                        for cut_i in range(len(r_rank)):
                            for idx, e in enumerate(sub_edge_idxs):
                                if elist[e][2] == r_rank[cut_i]:
                                    temp_e = entities_1[nodeidx][e].cpu().numpy()
                                    entities_1[nodeidx][e] = entities_0[nodeidx]
                                    relations_0[nodeidx][e] = 0
                            outputs,fea,fea_agg = self.model.module.forward(image, None, entities_0, entities_1,
                                                relations_0)
                            ...
                            if m_loss > best_loss:
                                for idx, e in enumerate(sub_edge_idxs):
                                    if elist[e][2] == r_rank[cut_i]:
                                        ...
                                        entities_1[nodeidx][e] = torch.from_numpy(temp_e)
                            else:
                                best_loss = m_loss
                    ...
                    self.entities_0=entities_0
                    self.entities_1=entities_1
                    self.relations_0=relations_0
                    self.model.module.prompt_learner.register_buffer("KGCN_entities_0", entities_0)
                    self.model.module.prompt_learner.register_buffer("KGCN_entities_1", entities_1)
                    self.model.module.prompt_learner.register_buffer("KGCN_relations_0",relations_0)
    ...
    @torch.no_grad()
    def test(self, entities_0=None, entity_1=None, relation_0=None, split=None, eval_no_Prune=None, loss_diff_t=None):
    ...
    def model_inference(self, input):
        return self.model.module.forward(input)
...
class TrainerX(SimpleTrainer):
    ...
    def run_epoch(self):
        ...
        for self.batch_idx, batch in enumerate(self.train_loader_x):
            data_time.update(time.time() - end)
            ##########################
            # https://github.com/xinliu20/MEC/blob/main/main_pretrain.py#L277
            ...
            lamda_schedule = self.lamda_scheduler(8 / lamda, 1 / lamda, self.max_epoch, self.num_batches, warmup_epochs=1)
            it = self.num_batches * self.epoch + self.batch_idx
            lamda_inv = lamda_schedule[it]
            ##########################
            loss_summary = self.forward_backward(batch,lamda_inv)
            batch_time.update(time.time() - end)
            losses.update(loss_summary)
